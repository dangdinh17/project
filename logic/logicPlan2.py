...
def sentence3() -> Expr:
    ...
    expr2 = ~conjoin(PacmanAlive_0, PacmanBorn_0)
    expr3 = PacmanBorn_0
...
def positionLogicPlan(problem) -> List:
    ...
    for timestep in range(50):
        ...
        if findModel(conjoin(KB) & PropSymbolExpr(pacman_str, xg, yg, time=timestep)) != False:
            return extractActionSequence(findModel(conjoin(KB) & PropSymbolExpr(pacman_str, xg, yg, time=timestep)),
                                         actions)
...
def foodLogicPlan(problem) -> List:
    ...
    "*** BEGIN YOUR CODE HERE ***"
    # Initial position and all the positions of the food into KB
    KB.append(PropSymbolExpr(pacman_str, x0, y0, time=0))
    for x, y in food:
...
def localization(problem, agent) -> Generator:
    ...
    for t in range(agent.num_timesteps):
        ...
        func1(t, all_coords, non_outer_wall_coords, walls_grid, sensorAxioms, allLegalSuccessorAxioms,
              fourBitPerceptRules, agent, KB)
...
def mapping(problem, agent) -> Generator:
    ...
    for t in range(agent.num_timesteps):
        """Call the helper functions that are implemented above question 6"""
        func1(t, all_coords, non_outer_wall_coords, known_map, sensorAxioms, allLegalSuccessorAxioms,
              fourBitPerceptRules, agent, KB)
        func3(known_map, non_outer_wall_coords, KB)
...
def slam(problem, agent) -> Generator:
    ...
    for t in range(agent.num_timesteps):
        """Call the helper functions that are implemented above question 6"""
        possible_locations = []
        func1(t, all_coords, non_outer_wall_coords, known_map, SLAMSensorAxioms, SLAMSuccessorAxioms,
              numAdjWallsPerceptRules, agent, KB)
        func3(known_map, non_outer_wall_coords, KB)
        func2(t, non_outer_wall_coords, possible_locations, KB)
